...
for x in $EXTENSIONS; do
  ...
  docker run --name oracle-build --shm-size=1G -p 1521:1521 -p 5500:5500 --env NO_WAIT=nowait --env ORACLE_SID=XE --env ENABLE_ARCHIVELOG=false --env ORACLE_PWD=root $BASE_IMAGE || {
  echo ""
  echo "ERROR: Oracle Database Docker Image container was NOT successfully run."
  exit 1
  }

  docker stop -t 600 oracle-build || {
  echo ""
  echo "ERROR: Oracle Database Docker Image container was NOT successfully stop."
  exit 1
  }

  docker commit -m "Image with prebuilt database" oracle-build $IMAGE_NAME || {
  echo ""
  echo "ERROR: Oracle Database Docker Image was NOT successfully commited."
  exit 1
  }

  docker rm oracle-build || {
  echo ""
  echo "ERROR: Oracle Database Docker Image container was NOT successfully deleted."
  exit 1
  }
